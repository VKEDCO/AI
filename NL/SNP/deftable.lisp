...
can add to the table with (put-state 'PR 'Puerto-Rico), or equivalently,
...
When it comes time to performance-tune your program, you can implement each 
table type as a hash table, property list, alist, or vector (if the keys
...
VK: I found this code on the web. It provides a nice implementation
of the table abstraction. The Semantic Network Processor uses deftable
quite a bit. I introduced several minor modifications not worth
commenting.
...
(defmacro deftable (name &rest args 
			 &key (type 'hash-table) inline default init
			 (tab (concatenate-symbols '* name '-table*))
			 (def (concatenate-symbols '* name '-table-default*))
			 (new (concatenate-symbols 'new- name '-table))
			 (put (concatenate-symbols 'put- name))
			 (get (concatenate-symbols 'get- name))
			 (clr (concatenate-symbols 'clear- name))
			 (map (concatenate-symbols 'map- name))
			 (rem (concatenate-symbols 'rem- name)) &allow-other-keys)
  ...
  new-x, put-x, get-x, clr-x, map-x rem-x, *x-table* and *x-table-default*).
...
(defun test-one (type)
  ...
  (labels ((show-table (msg)
	   ...
	   (key (x) (if (eq type 'vector) x (concatenate-symbols 'x x))))
